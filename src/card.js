...
Card.prototype.createToken = function (tokenData, callback) {
  tokenData.is_multiple_use =
    tokenData.is_multiple_use !== undefined ? tokenData.is_multiple_use : false;
  tokenData.should_authenticate =
    tokenData.should_authenticate !== undefined
      ? tokenData.should_authenticate
      : true;

  if (
    !tokenData.is_multiple_use &&
    (isNaN(tokenData.amount) || Number(tokenData.amount) < 0)
  ) {
    return callback({
      error_code: 'VALIDATION_ERROR',
      message: 'Amount must be a number equal or greater than 0',
    });
  }
  ...
  if (!CreditCardUtil.isCreditCardCVNValid(tokenData.card_cvn)) {
    return callback({
      error_code: 'VALIDATION_ERROR',
      message: 'Card CVN is invalid',
    });
  }

  if (
    !CreditCardUtil.isCreditCardCVNValidForCardType(
      tokenData.card_cvn,
    ...
    return callback({
      error_code: 'VALIDATION_ERROR',
      message: 'Card CVN is invalid for this card type',
    });
  }
...
Card.prototype.createTokenV1 = function (
  transactionData,
  transactionMetadata,
  callback
) {
  ...
  transactionData.is_multiple_use =
    transactionData.is_multiple_use !== undefined
      ? transactionData.is_multiple_use
      : false;
  transactionData.should_authenticate =
    transactionData.should_authenticate !== undefined
      ? transactionData.should_authenticate
      : true;
  ...
  if (
    !transactionData.is_multiple_use &&
    (isNaN(transactionData.amount) || Number(transactionData.amount) < 0)
  ) {
    return callback({
      error_code: 'VALIDATION_ERROR',
      message: 'Amount must be a number equal or greater than 0',
  ...
  if (
    !CreditCardUtil.isCreditCardExpirationDateValid(
      transactionData.card_exp_month,
      transactionData.card_exp_year
    )
  ) {
    return callback({
      error_code: 'VALIDATION_ERROR',
  ...
  this._getTokenizedCreditCard(
    transactionData,
    function (err, tokenizedCreditCard) {
      if (err) {
        return callback(err);
      }

      if (transactionMetadata !== null) {
        transactionMetadata.credit_card_type = CreditCardUtil.getCardType(
          transactionData.card_number
        );
        transactionMetadata.device_fingerprint_id =
          self._xendit._device_fingerprint_id;
      }

      self._createCreditCardTokenV1(
        tokenizedCreditCard.token,
        transactionData,
        transactionMetadata,
        function (err, creditCardCharge) {
          if (err) {
            return callback(err);
          }
...
Card.prototype.createAuthentication = function (
  authenticationData,
  transactionMetadata,
  callback
) {
  ...
  if (
    isNaN(authenticationData.amount) ||
    Number(authenticationData.amount) < 0
  ) {
    return callback({
      error_code: 'VALIDATION_ERROR',
      message: 'Amount must be a number equal or greater than 0',
...
Card.prototype._tokenizeCreditCard = function (
  tokenizationConfiguration,
  transactionData,
  callback
) {
  var baseFlexUrl =
    this._xendit._getEnvironment() === 'PRODUCTION'
      ? tokenizationConfiguration.flex_production_url
      : tokenizationConfiguration.flex_development_url;
...
Card.prototype._createCreditCardToken = function (tokenData, callback) {
  ...
  var body = {
    ...
    card_data: {
      ...
      card_holder_first_name: tokenData?.card_holder_first_name,
      card_holder_last_name: tokenData?.card_holder_last_name,
      card_holder_email: tokenData?.card_holder_email,
    },
    should_authenticate: tokenData.should_authenticate,
  };

  if (
    (!body.is_single_use && body.card_data.cvn === '') ||
    body.card_data.cvn === null
  ) {
  ...
  if (
    tokenData.card_holder_first_name !== undefined &&
    tokenData.card_holder_first_name !== ''
  ) {
    body.card_holder_first_name = tokenData.card_holder_first_name;
  }

  if (
    tokenData.card_holder_last_name !== undefined &&
    tokenData.card_holder_last_name !== ''
  ) {
    body.card_holder_last_name = tokenData.card_holder_last_name;
  }

  if (tokenData.card_cvn !== undefined && tokenData.card_cvn !== '') {
    body.card_holder_email = tokenData.card_holder_email;
  }
...
Card.prototype._createCreditCardTokenV1 = function (
  creditCardToken,
  transactionData,
  transactionMetadata,
  callback
  ...
  if (
    transactionData.card_cvn !== undefined &&
    transactionData.card_cvn !== ''
  ) {
...
Card.prototype._createAuthentication = function (
  authenticationData,
  transactionMetadata,
  callback
) {
  ...
  var body = {
    ...
    currency: authenticationData?.currency || undefined,
  };
